...
#SimSiam transform taken from https://github.com/PatrickHua/SimSiam/blob/main/augmentations/simsiam_aug.py
class SimSiamTransform():
    def __init__(self, image_size= 28, mean=None,std=None):
        
        p_blur = 0.5 if image_size > 32 else 0 # if image is too small
        ...
        self.transform = T_transforms.Compose([
            ...
            T_transforms.RandomApply([T_transforms.ColorJitter(0.4,0.4,0.4,0.1)], p=0.8),
            T_transforms.RandomGrayscale(p=0.2),
            T_transforms.RandomApply([T_transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0))], p=p_blur),
...
def get_dataset(train_percent = 0.1,root_dir = '../data'):
    ...
    ssl_transform = SimSiamTransform(mean = mean,std = std)
    ...
    unlabel_set = DatasetFromSubset(unlabel_set,transform =  SimSiamTransform(mean = [0.7405, 0.5330, 0.7058],std = [0.1237, 0.1768, 0.1244]))
